...
Graph.prototype.updateLayout = function() {
	// for(i=0;i<this.iteration;i++){
		// this.stepsize=this.stepsize*this.cooldown;
	for (i in this.vertices) {
		...
		for (j in this.vertices) {
			if( i !== j ) {
				...
				if(distance<400){   //ignore repulsion from remote vertex
	...
	for (i in this.vertices) {
		...
		// if(this.vertices[i].posx>this.temperature)
			// this.vertices[i].posx=this.temperature;
		this.vertices[i].posy += this.forcey[i] * this.stepsize;
		// if(this.vertices[i].posy>this.temperature)
			// this.vertices[i].posy=this.temperature;
	...
	this.iteration++;
	//this.stepsize=this.stepsize*this.cooldown;
	if( this.iteration > 2000 ) // XXX -- should watch for rest state, not just quit after N iterations
		this.quit();
}
